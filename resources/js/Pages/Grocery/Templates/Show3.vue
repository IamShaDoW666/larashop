      ...
      <FrontEnd :store="store">
        ...
        <div class="flex px-4 items-center md:mr-4 md:ml-8 my-2 justify-between">
          <div class="sm:flex sm:space-x-4 items-center">
            <div class="flex space-x-2 items-center">
              <span class="rounded px-2 my-2 text-white dark:text-black font-bold overflow-visible"
                :class="store.open_status ? 'bg-green-400 dark:bg-golden-yellow' : 'bg-blood-red'">
                {{ store.open_status ? __('Open') : __('Closed') }}
              </span>
              <span :class="store.open_status ? 'bg-green-800 dark:bg-golden-yellow' : 'bg-blood-red'"
            ...
            <span class="dark:text-white">{{ store.address }}</span>
          ...
          <div class="flex gap-y-4 sm:gap-y-0 gap-x-4">
            ...
            <Instagram v-if="store.instagram" :link="store.instagram" />
          </div>
        </div>
<div class="input-group relative flex rounded-md flex-between items-stretch gap-2 w-full md:w-full px-8 md:px-8 mb-4"
                >
                    <input
        ...
        <div ref="category_slider" class="
            ...
            no-scrollbar
          ">
          <div v-if="store.categories" class="
              justify-start
              mx-auto
              flex
              items-center
            ...
            <div @click="categoryFilter(category.id)" :class="[
              active[category.id]
                ...
                : 'bg-green-300',
            ]" v-for="category in store.categories" :key="category.id" class="
                p-2
                shrink-0
                cursor-pointer
                rounded
        ...
        <ProductsList :prod="prod" :filteredProducts="filteredProducts"  :openModal="openModal" :store="store" :toCart="toCart" />

        <CartModal v-if="cart.items.length" :store="store" :checkout="checkout" :cart="cart" :formatPrice="formatPrice"
          :fromCart="fromCart" />
      </FrontEnd>
    </div>
                ...
                <div class="flex flex-col space-y-4">
                  <div class="
                    ...
                    <div v-if="cart.getTaxValue" class="text-sm">
                      {{ store.config.tax_name }}({{ store.config.tax }}%): {{ formatPrice(cart.getTaxValue)
                      }}
                    </div>
...
import ProductsList from "@/Components/grocery/ProductsList.vue";
...
export default {
  ...
  setup(props) {
    const { filter, prod,searchQuery, filteredProducts,resetCategory, active } = useProducts(props);
    const cart = useCart();
    ...
    const checkout = () => {
      form.get(route("orders.checkin", { grocery: props.store.id }));
    };
