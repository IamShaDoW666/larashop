...
const center = reactive({
    ...
    lng: Number(usePage().props.value.auth.restorant.lng),
})

const setLatLng = async () => {
    await axios.post(route('owner.restorant.update-location', { restorant: usePage().props.value.auth.restorant.id }), center).then((res) => {
