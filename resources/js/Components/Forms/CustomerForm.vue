    ...
    <div class="mt-8 md:mt-0 bg-white rounded-lg shadow-lg p-8">
        ...
        <form method="POST" @submit.prevent="submit" class="flex flex-col">
            ...
            <input
                ...
                type="text"
            />
            <label class="font-semibold text-xs mt-3" for="phonenumber"
                >Phone Number</label
            >
            <input
                ...
                type="text"
            />
            <label class="font-semibold text-xs mt-3 mb-2" for="address"
                >Order type</label
            >
            ...
            <label v-if="form.order_type == 1" class="font-semibold text-xs mt-3" for="address"
                >Address</label
            >
            <textarea
            ...
            <span v-if="areas.length && form.order_type == 1" class="font-bold"
            ...
            >
                ...
                >
                    <span
                        class="block text-sm py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
            ...
            <button
                ...
                class="flex items-center justify-center gap-x-2 h-12 px-6 w-64 bg-green-600 mt-8 rounded font-semibold text-sm text-green-100 hover:bg-green-700"
            >
        ...
        <button
            ...
            class="flex items-center justify-center h-12 px-6 w-64 bg-green-600 mt-8 rounded font-semibold text-sm text-green-100 hover:bg-green-700"
        >
            Pay with Razorpay
        </button>
...
const razorpay = () => {
    ...
    axios
        .post(route("debug.post"), {
            restorant: usePage().props.value.auth.restorant.id,
        ...
        .then((res) => {
            ...
            const options = {
                ...
                currency: usePage().props.value.auth.restorant.currency,
                name: usePage().props.value.auth.restorant.name,
                description: "",
                image: `${usePage().props.value.app.url}${
                    usePage().props.value.auth.restorant.logo
