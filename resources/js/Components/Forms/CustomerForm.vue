    ...
    >
        ...
        <form method="POST" @submit.prevent="submit" class="flex flex-col">
            ...
            <label
                ...
                >Order type</label
            >                        
            <div v-if="!$page.props.table || $page.props.table === null" class="flex gap-x-2">
                <div
                    class="form-control"
                    v-if="Boolean(Number(grocery.config.can_deliver))"
                >
                ...
                <div
                    class="form-control"
                    v-if="Boolean(Number(grocery.config.can_pickup))"
                >
                    <label class="label cursor-pointer">
                        <span class="label-text mr-2 dark:text-white"
                            >Pickup</span
                ...
                <div
                    class="form-control"
                    v-if="Boolean(Number(grocery.config.can_dinein))"
                >
                    <label class="label cursor-pointer">
                        <span class="label-text mr-2 dark:text-white"
                            >Dine-in</span
            ...
            <div v-else>
                <label class="label cursor-pointer">
                        <span class="label-text mr-2 dark:text-white text-md"
                            >Dine-in Table: </span
                        >
                        <span>{{ $page.props.table.name + ' : ' + $page.props.table.id }}</span>
...
const razorpay = () => {
    ...
    axios
        .post(route("pay.razorpay"), {
            grocery: props.grocery.id,
            amount: cart.getTotal,
        })
        .then((res) => {
            loading.value = false;
            const options = {
                key: props.grocery.config.razorpay_api_key,
                amount: cart.getTotal * 100,
                currency: props.grocery.currency,
                name: props.grocery.name,
                description: "",
                image: `${usePage().props.value.app.url}${
                    props.grocery.logo
                }_logo.webp`, //${usePage().props.value.app.url}${usePage().props.auth.grocery.logo}_logo.webp
                order_id: res.data,
                handler: function (response) {
                    Inertia.post(
                        route("orders.store", {
                            grocery: props.grocery.uuid,
                        }),
                        {
                            form,
...
const submit = () => {
    Inertia.post(route("orders.store", { grocery: props.grocery.uuid }), {
        form,
        payment_method: "cod",
        cart: props.cart.getCart(),
    }),
...
onMounted(() => {
    loading.value = false;
    if (usePage().props.value.table && usePage().props.value.table != null) {
        form.order_type = 3;        
    }
