<template>
    <div class="mt-8 md:mt-0 bg-white rounded-lg shadow-lg p-8">
        ...
        <form method="POST" @submit.prevent="submit" class="flex flex-col">
            ...
            <label v-if="form.order_type == 1" class="font-semibold text-xs mt-3" for="address">Address</label>
            ...
            <select v-if="areas.length && form.order_type == 1" v-model="cart.delivery"
                ...
                <option placeholder="Delivery Area" aria-placeholder="Delivery Area" :value="area.delivery_fee"
                    ...
                    <span class="block text-sm py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{
                            area.name
                    }} {{ area.delivery_fee }}</span>
                </option>
            </select>
            <label v-if="form.order_type == 1" class="font-semibold text-xs mt-3" for="delivery_time">
                Delivery Time
            </label>
            <label v-if="form.order_type == 3" class="font-semibold text-xs mt-3" for="dinein_time">
                Dine-in Time
            </label>
            ...
            <div class="flex items-center my-4 px-4">
                ...
                <label for="default-checkbox" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">I agree
