<template>
  <div class="sm:flex block gap-x-3 justify-between">
    <div class="container flex flex-col w-full sm:w-3/5">
      <div
        class="px-4 py-5 sm:px-6 w-full flex justify-between border dark:bg-gray-800 bg-white shadow mb-2 rounded-md">
        <div>
        ...
        <div v-if="cart.items.length > 0" class="text-md font-bold text-green-700">
          <span class="text-black">Amount Payable: </span>{{ formatPrice(cart.getTotal.toFixed(2)) }}
        </div>
        <div class="font-bold text-lg">
          Total Items: {{ formatPrice(cart.getTotalItems) }}
        </div>
      </div>
