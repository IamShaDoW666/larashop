<template>
    <TransitionRoot appear :show="editPlan" as="template">
        <Dialog
            as="div"
            @close="closeModal"
...
const { editPlan, dPlan } = inject("data");

const form = useForm({
    name: dPlan.value.name,
    price: dPlan.value.price_int,
    items: dPlan.value.items,
});
...
const submit = () => {
    form.patch(route("super.plans.update", {plan :dPlan.value.id}), {
        onSuccess: () => {
            form.reset();
            closeModal();
            Swal.fire({
