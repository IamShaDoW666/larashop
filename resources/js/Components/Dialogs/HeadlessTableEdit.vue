<template>
    <TransitionRoot appear :show="tableOpenEdit" as="template">
                ...
                <div class="mt-4 px-6 pb-6">
                  <form>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-2 gap-y-2">
                      <div>
                        <BreezeLabel for="name" value="Table Name" />
  ...
  <!--
                      ...
                      <div>
                        <BreezeLabel for="price" value="Table Number" />
                        <input id="number" type="number" class="mt-2 border-gray-300 w-full sm:w-2/3 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" v-model="form.number" required>
                      </div>
  ...
  const { tableOpenEdit, dTable } = inject('data')
  
  const form = useForm({
    name: dTable.value.name,
    number: dTable.value.number
  })
  
  function closeModal() {
    tableOpenEdit.value = false
  }
  function openModal() {
  ...
  const submit = () => {
    form.patch(route('tables.update', { table: dTable.value.id }), {
      onSuccess: () => {
        form.reset()
        closeModal()
        Swal.fire({
