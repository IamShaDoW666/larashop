<template>
    <TransitionRoot appear :show="variantEditOpen" as="template">
...
const { variantEditOpen, dVariant } = inject('data');


const form = useForm({
    name: dVariant.value.name,
    price: dVariant.value.price_int
})


function closeModal() {
    variantEditOpen.value = false
}
function openModal() {
    variantEditOpen.value = true
}

const submit = () => {
    form.patch(route('variant.update', { variant: dVariant.value.id }), {
        onSuccess: () => {
            form.reset()
            closeModal()
            Swal.fire({
