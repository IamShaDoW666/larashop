    ...
    <GroceryLayout>
        <div class="rounded-t bg-white mb-0 px-6 py-6">
            <div class="text-center flex justify-between">
                <h6 class="text-gray-700 text-xl font-bold">{{ __('store Settings') }}</h6>
                <Link :href="
                    route('grocerys.show', {
                ...
                <div class="flex flex-wrap">
                    <div class="w-full lg:w-4/12 px-4">
                        ...
                        <div class="relative w-full mb-3">
                            <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2"
                                htmlFor="grid-password">
                                {{ __('store Banner') }} {{ __('1920x1280 image size') }}
                            </label>
...
import GroceryLayout from '@/Layouts/Grocery';
...
let conf = usePage().props.value.auth.grocery.config;
console.log(grocery);
const previewImage = ref(`${grocery.banner}_large.webp`);
const logoPreviewImage = ref(`${grocery.logo}_logo.webp`);
const fileInput = ref();
...
const form = useForm({
    ...
    phone: grocery ? grocery.phone : "",
    minimum_order: conf ? conf.minimum_order : 0,
    country: grocery ? grocery.country : "",
    address: grocery ? grocery.address : "",
    ...
    city: grocery ? grocery.city : "",
    postal_code: grocery ? grocery.postal_code : "",
    currency: conf ? conf.currency : "",
    banner: grocery ? grocery.banner : "",
    logo: grocery ? grocery.logo : "",
    instagram: grocery ? grocery.instagram : "",
    facebook: grocery ? grocery.facebook : "",
    twitter: grocery ? grocery.twitter : "",
...
const update = () => {    
    Inertia.post(
        route("owner.grocery.update", { grocery: grocery.id }),
        form,
        {
            onSuccess: () => {
                Swal.fire({
